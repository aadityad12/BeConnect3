...
This repo now includes a lightweight UI wrapper at `pi_sender/streamlit_app.py`.
It uses the existing `beconnect_pi` model/storage/broadcast logic; no new data layer is introduced.
...
- Create/save alerts
- Publish selected alert to `current_alert.json`
- Start/stop BLE broadcaster
- View broadcaster status + log tail
